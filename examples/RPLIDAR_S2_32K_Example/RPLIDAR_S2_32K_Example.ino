//RPLIDAR S2 32K Scan Mode Example using the RPLidarCV library
...
// Also, change the LIDAR's serial port on line 34 and 108 to match your setup (Serial1 is currently set)
...
void setup() {
  ...
  Serial.println("RPLIDAR S2 Example for 32K Scan Rate");
  Serial.println();
  ...
  //->>>>> Change the serial port here and on line 108 if you use a different one
  ...
  //Start scanning in 32K mode (stop if scan start fails)
  if (!IS_OK(rplidar.startScanMode(RPLIDAR_SCAN_MODE_32K))) {
    Serial.println("LIDAR Failed to Start");
    rplidar.setMotorSpeed(0);
    rplidar.stop();
    rplidar.end();
    while(1) {};
  } else Serial.println("Start Scan - 32K Mode");
...
void RecordLIDAR_RP_32K() {
  boolean done = false;
  int rot_count = 0;
  float cur_angle_deg;
  int cur_distance_mm;
  rplidar_response_capsule_measurement_nodes_t    capsule_node;
  rplidar_response_measurement_node_hq_t   local_buf[256];
  size_t                                   count = 256;
  rplidar_response_measurement_node_hq_t   local_scan[4096];
  size_t                                   scan_count = 0;
  u_result                                 ans;
  ...
  rplidar.waitCapsuledNode(capsule_node); // // always discard the first data since it may be incomplete
  ...
  while (done==false) {
    ...
    // Read LIDAR Data
      if (IS_FAIL(ans=rplidar.waitCapsuledNode(capsule_node))) {
      ...
      //Convert capsuled input into individual readings
      rplidar.denseCapsuleToNormal(capsule_node, local_buf, count);
